...
<c:url var="addSearch" value="/home/login/bookings/search" />
...
<sf:form method="post"
	action="${pageContext.request.contextPath}/home/login/bookings/search"
	modelAttribute="form">
...
<div class="form-outline ">
			...
			<table class="table bg-header rounded shadow-sm table-hover mt-5">
				<thead class="bg-footer text-white">
					<tr>
						...
						<c:if
							test="${sessionScope.user.roleId == 2}">
							<th scope="col">Status</th>
 ...
 --%>
							...
							<c:if test="${sessionScope.user.roleId == 3}">
								...
								<c:if test="${sessionScope.user.roleId == 3 && s_list.status eq 'accept'}">
									
									<td><a class="text-dark"
										href="${editUrl} ${s_list.user.id}"><i class="fas fa-edit"></i></a></td>
										<td><a class="text-primary"
										href="${s_list.user.paymentLink}" target="_blank">${s_list.user.paymentLink}</a></td>
								</c:if>
							...
							<c:if test="${sessionScope.user.roleId == 2}">
									<c:if test="${s_list.status eq 'accept' || s_list.modifiedBy!=null}">
									<td><a class="nav-link "
