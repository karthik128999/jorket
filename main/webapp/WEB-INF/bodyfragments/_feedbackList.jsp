...
<c:url var="addUrl" value="/home/login/users" />

<c:url var="addSearch" value="/home/login/feedback/search" />
...
<sf:form method="post"
	action="${pageContext.request.contextPath}/home/login/feedback/search"
	modelAttribute="form">
				...
				<tbody>
					...
					<c:forEach items="${list}" var="s_list" varStatus="u">
						<tr>
							...
							<c:if test="${sessionScope.user.id == s_list.userId || sessionScope.user.roleId==3}">
							
							<c:if test="${s_list.ratings==1}">
							<td scope="row">&#11088</td>
							</c:if>
							<c:if test="${s_list.ratings==2}">
							<td scope="row">&#11088 &#11088</td>
							</c:if>
							<c:if test="${s_list.ratings==3}">
							<td scope="row">&#11088 &#11088 &#11088</td>
							</c:if>
							<c:if test="${s_list.ratings==4}">
							<td scope="row">&#11088 &#11088 &#11088 &#11088</td>
							</c:if>
							<c:if test="${s_list.ratings==5}">
							<td scope="row">&#11088 &#11088 &#11088 &#11088 &#11088</td>
							</c:if>
							
							<td scope="row">${s_list.comments}</td>
