...
<c:url var="addSearch" value="/home/login/users/search" />
...
<sf:form method="post"
	action="${pageContext.request.contextPath}/home/login/users/search"
	modelAttribute="form">
				...
				<tbody>
					...
					<c:forEach items="${list}" var="s_list" varStatus="u">
						...
						<c:if test="${s_list.roleId != 1}">
							...
							<td>
							<c:if test="${s_list.roleId==2 }">
							<a class="btn btn-sm btn-danger" target="_blank" href="${kycUrl}${s_list.id}">Download</a>
							</c:if>
							
							<c:if test="${s_list.roleId==3 }">
							<span class="text-dark" align="center">-</span>
							</c:if> </td> 
							</c:if>
						</tr>
					</c:forEach>
