...
<c:url var="editUrl" value="/home/login/payment?id=" />   
...
<div class="card" >
  <div class="card-body bg-header" >
    ...
    <sf:form method="post" action="${pageContext.request.contextPath}/home/login/booking" enctype="multipart/form-data" modelAttribute="form"><br/>
 
 
 	<input type="hidden" value="${serviceId }" name="serviceId">
 	<c:if test="${form.status eq 'requestbooking'}">
 		<div class="container">
 			<div class="alert alert-danger text-center text-dark fw-bold">Wait for Professional
 			to accept the request</div>
 		
 			<a href="${editUrl} ${pk}" >Check Payment Page</a>
 		</div>
 	</c:if>
    <c:if test="${form.status eq 'accept'}">
    here
