...
<c:url var="addSearch" value="/home/login/booking/search" />
...
<sf:form method="post"
	action="${pageContext.request.contextPath}/home/login/booking/search"
	modelAttribute="form">
	<div class="container mt-4"
		...
		<div class="form-check d-flex justify-content-center mb-4 mt-2">
			<input type="submit" name="operation"
				class="btn btn-primary  pull-right" value="Filter"> &nbsp; <input
		...
		<div class="table-responsive">
			...
			<table 
				class="table bg-header rounded shadow-sm table-hover mt-5">
				...
				<tbody>
					...
					<c:forEach items="${list}" var="s_list" varStatus="u">
						<tr>
							...
							<c:if
								test="${sessionScope.user.roleId == 3 || sessionScope.user.roleId == 1 || sessionScope.user.id==s_list.user.id || sessionScope.user.login==s_list.createdBy}">
								...
								<c:if test="${sessionScope.user.roleId == 2 }">
									<c:if test="${s_list.status eq 'accept'}">
										<td><a class="text-white btn btn-success"
								...
								<c:if test="${sessionScope.user.roleId == 2}">
									<c:if
										...
										href="<c:url value="/home/login/feedback/search"/>?fid=${s_list.createdBy}">View
											Feedback</a></td> --%>
										<c:if test="${s_list.user.feedbackDTOList[0].ratings==1}">
											<td scope="row">&#11088</td>
										</c:if>
										<c:if test="${s_list.user.feedbackDTOList[0].ratings==2}">
											<td scope="row">&#11088 &#11088</td>
										</c:if>
										<c:if test="${s_list.user.feedbackDTOList[0].ratings==3}">
											<td scope="row">&#11088 &#11088 &#11088</td>
										</c:if>
										<c:if test="${s_list.user.feedbackDTOList[0].ratings==4}">
											<td scope="row">&#11088 &#11088 &#11088 &#11088</td>
										</c:if>
										<c:if test="${s_list.user.feedbackDTOList[0].ratings==5}">
											<td scope="row">&#11088 &#11088 &#11088 &#11088 &#11088</td>
										</c:if>

										<td>${s_list.user.feedbackDTOList[0].comments }</td>
									</c:if>
